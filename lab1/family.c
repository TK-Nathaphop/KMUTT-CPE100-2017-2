 ...
 *  relationships.
 *
 *  Created by Sally Goldin, 8 January 2014 for CPE113
 *  Updated 9 January 2016 to add more comments.
 *  Updated 24 December 2016 to fix crash from inappropriate free if
 *     file does not exist, and to exit if empty name. 
 */
    ...
    switch (relation)
       ...
       case REL_CHILD:
           ...
           if (strcmp(pPersonB->gender,"F") == 0) 
            ...
            if (pPersonA->pMother == NULL)
	            ...
	            {
		          printf("Cannot set %s to be the child of %s because %s already has a mother\n",  
              ...
              status = 0;
	           ...
	           {
		          printf("Cannot set %s to be the child of %s because %s already has a father\n",  
		          pPersonA->name, pPersonB->name, pPersonA->name);
                   status = 0;
	           } 
       ...
       case REL_FATHER: 
           if (strcmp(pPersonA->gender,"M") == 0)   /* A must be a man to be a father */
	       {
	           ...
	           {
		      printf("Cannot set %s to be the father of %s because %s already has a father\n",  
		      pPersonA->name, pPersonB->name, pPersonB->name);
                   status = 0;
	           } 
	       }
