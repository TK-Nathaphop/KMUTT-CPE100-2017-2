 ...
 *
 *  Created by Sally Goldin, 8 February 2012, for CPE 113
 */
...
void* heapExtract()
{
    ...
    if ((treeArray != NULL) && (size > 0))
	   ...
	   rc = right(ip);
           ...
           if ((lc < size) &&
               (reverse * compareFn(treeArray[lc].data,treeArray[ip].data) < 0))
	       {
	       swapp = lc;           
	       }
           else
           ...
           if ((rc < size) &&
               (reverse * compareFn(treeArray[rc].data,treeArray[swapp].data) < 0))
	       {
	       swapp = rc;
	       }
