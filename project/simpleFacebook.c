 ...
 * simpleFacebook.c
 *
 *      Main program of Simple Facebook.
 *      This program will act as a social network that named Facebook.
 *
 *      It working with 4 main data structures.
 *      There are hash table, AVL binary tree, graph and linked list.
 *
 *      Hash table and AVL Binary tree are the main data structure
 *      used to keep all of user
 *      
 *      Graph is used to manage relation of friend and pending request
 *
 *      Linked list is used to keep status and comment of each user
 *
 ...
 * Create a new user then ask user for input information
 * After input finished, keep in data structure.
 */
void controlRegister()
    {
    USER_T *pNew = (USER_T*)calloc(1,sizeof(USER_T));   /* New data */
    if (pNew == NULL)
        printError(1);
    headUserInterface();
    printf("REGISTER:\n");
    ...
    do
        ...
        if(findData(pNew->mail) != NULL)
            printf("#SYSTEM: This E-mail have been register already.\n");
        else
            break;
        }while(1);
...
/* Control sign in part.
 * Loop ask user to input E-mail and password then find in data structure
 * If found and password correct, let user login in to system.
...
/* Control facebook part
 * Working after user can sign in into Simple Facebook.
...
void controlFacebook()
    ...
    char input[LEN] = {0};      /* Get input from user */
    char text[LONGLEN] = {0};   /* Get text after command */
 ...
 * Send text and current date to update comment in database
 * If update success print new comment show user.
 * But if not, print error.
 */
void controlUpdateComment(char *pText)
    {
...
/* Control see pending request part.
 * Print information that send pending request to user login
 * Then ask user login that want to add to be friend or not.
 */
void controlPending()
    {
    int count = 0;
 ...
 * Arguments:
 *      select - Select to add or delete friend
 *      find - Name that we want to find.
 */
void controlFriend(int select, USER_T *pUser)
    {
...
/* Control home command
 * Refresh every thing like when user just sign in.
 * Clear screen, print suggest friend and show the lastest status.
 */
void controlHome()
    {
    controlHeadStatus(1);
 ...
 * If user want to exit dump and free all data.
 * But if not, just go back.
...
/* Control function to display suggest friend
 */
void controlSuggestFriend()
    {
    int i = 0;         /* For loop */
