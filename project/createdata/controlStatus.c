 ...
 * controlStatus.c
 *
 *      Control data structure that related to status.
 *
...
static STATUS_T ** _status = NULL; /* Status that we want to  */
static int _arrayBox = 0;            /* Amount of friend */

/***************************** LOCAL FUNCTION ********************************/

/* LOCAL FUNCTION.
 * This function swap position of data
 * Arguments:
...
void freeStatus(STATUS_T *pHead)
    {
    STATUS_T *pTemp = pHead;    /* Temporary status */
    STATUS_T *pDel = pHead;     /* Status that we want to delete */
    while(pDel != NULL)
        {
        pTemp = pDel->pNext;
        _freeCommentList(pDel -> pHeadC);   /* Free comment */
        pDel -> pHeadC = NULL;
        pDel -> pTailC = NULL;
        free(pDel);
        pDel = pTemp;
...
void addStatus(USER_T *pUser, STATUS_T *pNew, int select)
    ...
    if(select == 1) /* Add to head node */
        ...
        pTemp = pUser->pTailS;
        pUser->pTailS = pNew;
        if(pTemp != NULL)   /* Set pointer of status before the last to new */
            pTemp->pNext = pNew;
        else                /* Otherwise, that mean user not have any status */
            pUser->pHeadS = pNew;
        }
    }
...
void addComment(STATUS_T* pStatus, COMMENT_T *pNew)
    {
    COMMENT_T *pTemp = pStatus->pTailC;
    pStatus->pTailC = pNew;
    if(pTemp != NULL)
        pTemp->pNext = pNew;
    else
