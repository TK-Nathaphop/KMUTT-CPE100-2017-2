...
/* LOCAL FUNCTION.
 * Check the tld (tld is a .net,.com,.co.th etc.) of mail from user.
 * This program will check whether the tld of the program exist or not.
 * Arguments:
 ...
 */
int _checkPhoneNumber(char string[128])
    {
    if(string[1] == '6')
        {
...
COMMAND_TYPE checkCommand(char *pCheck, char *pReturn)
    ...
    char retText[LONGLEN] = {0};          /* Text that want to return */
    ...
    int i = 0;              /* Count loop anc check equipment */
        ...
        if(strcasecmp(pCheck, command[i]) == 0)
            ...
            /* For command that have text follow */
                ...
                if (pCut == NULL)
                    ...
                    if(strlen(pCut) == 0)
                        return C_ERROR;
                    strncpy(retText, pCut, sizeof(retText));
                    strcpy(pReturn, retText);
                    }
                }
...
int validatePhone(char *pString)
    ...
    if (!_checkDigit(pString))          /* Phone number is not digit */
        return 3;
    if (!_checkPhoneNumber(pString))   /* Phone number is not Thai number */
        return 4;
    return 1;
    }
 ...
 *     date - Date to check
 * date[0] = day, date[1] = month, date[2] = year
 * Return 0 if date is after current  (Invalid)
 * Return 1 if date is before current (Valid)
 */
int compareBirthDate(int date[])
    {
    int bOk = 0;
...
/* Validate date from string it's correct or not.
 * Argument:
 *     pString - Keep name of user that we want to validate
 * Return 0, if date is incorrect.
 * Return 1, if date is correct.
 */
int validateBirthString(char *pString)
    {
    int i = 0;          /* Count loop */
    int date[3] = {0};  /* Date */
    int dummy = 0;      /* Dummy to check input */
    int countSpace = 0;     /* Count spacebar */
    for(i = 0; i < strlen(pString); i++)
        {
        if (pString[i] == ' ') /* Count space */
            countSpace++;
        /* Check not number */
        else if (!isdigit(pString[i]) && !isspace(pString[i]))
            return 0;
        }
    if (countSpace != 2) /* Space shouldn't moer than or less than 2 */
        return 0;
    else if(strstr(pString, "  ") != NULL) /* Check space next to space or not */
        return 0;
    /* Check that user input 3 numbers or not */
    else if (sscanf(pString,"%d %d %d %d", &date[0], &date[1],
                                           &date[2], &dummy) != 3)
        return 0;
    return compareBirthDate(date);
    }

/* Validate password of user.
 * Argument:
 *      pString - Keep password that we want to validate
 * Return 0, if password is wrong format.
 * (Have 1 uppercase, 1 lowercase and 1 number)
...
/* Validate all of data in user.
 * ARGUMENTS: 
 *      pData - Data that want to validate
 */
int validateUser(USER_T *pData)
    {
    if (validateMail(pData->mail) != 1)
        return 0;
    else if (validatePhone(pData->phone) != 1)
        return 0;
    else if (validateName(pData->name) != 1)
        return 0;
    else if (compareBirthDate(pData->dateBirth) != 1)
        return 0;
    else if (validatePassword(pData->password) != 1)
        return 0;
    return 1;
    }