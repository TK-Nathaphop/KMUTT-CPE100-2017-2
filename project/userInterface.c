 ...
 *
 *      Created by Nathaphop Sundarabhogin (KLA)    ID : 60070503420
 *      18 April 2018
 *
 ******************************************************************************
 */
...
void printHelp()
    ...
    printf("\nAfter command '/pending' user can only answer.\n");
    printf("'Y' for accept pending request to be friend.\n");
    printf("'N' for deny pending request to not be friend.\n");
    printf("Otherwise, hit<CR> to go back to main function.\n");
...
/* Print comment.
 * Argument:
 *      pComment - Comment that we want to print.
 */
void printComment(COMMENT_T *pComment)
    {
    USER_T *pOwner = NULL;      /* Owner of comment */
    char *pText = {0};          /* Text of comment */
    int *pDate = NULL;          /* Date of comment */
...
int askMail(char *pReturn)
    ...
    while(retVal != 1)
        ...
        if (strlen(input) == 1)
            return 0;
        cutLine(input);
        retVal = validateMail(input);
        if (retVal == 0)
            printf("#SYSTEM: Not allowed to uses this E-mail\n");
        }
...
int askPassword(char *pReturn)
    ...
    while(retVal != 1)
        ...
        if (strlen(input) == 1)
            return 0;
        cutLine(input);
        retVal = validatePassword(input);
        if(retVal == 0)
            {
            printf("#SYSTEM: Password must contain 1 uppercase letter, ");
            printf("1 lowercase, and 1 number.\n");
            }
        else if (retVal == 2)
            printf("#SYSTEM: Length of password should be 8 - 16 characters.\n");
        else if (retVal == 3)
            printf("#SYSTEM: Can't use special symbol\n");
        }
    strcpy(pReturn, input);
    return 1;
    }

/* Ask and validate name
 * Argument:
...
int askName(char *pReturn)
    ...
    while(retVal != 1)
        ...
        if (strlen(input) == 1)
            return 0;
        cutLine(input);
        retVal = validateName(input);
        if (retVal == 0)
            {
            printf("#SYSTEM: There are something wrong in name.\n");
            printf("#SYSTEM: Please try again\n");
            }
        }
    strcpy(pReturn, input);
...
int askPhone(char *pReturn)
    ...
    while(retVal != 1)
        ...
        if (strlen(input) == 1)
            return 0;
        cutLine(input);
        retVal = validatePhone(input);
        if (retVal == 2)
            printf("#SYSTEM: Phone number is wrong format\n");
        else if (retVal == 3)
            {
            printf("#SYSTEM: Can't use alphabet or special character");
            printf(" in phone number\n");
            }
        else if (retVal == 4)
            {
            printf("#SYSTEM: Wrong format of Thai phone number\n");
            printf("#SYSTEM: Thai phone number must starts with ");
            printf("'066', '067', '069', '08' and '09'\n");
            }
        }
    strcpy(pReturn, input);
    return 1;
...
int askBirthDate(int pReturn[])
    ...
    while(retVal != 1)
        {
        printf("Birth Date (date month year)\n");
        printf("(Example: 8 9 1998): ");
        fgets(input, sizeof(input), stdin);
        if (strlen(input) == 1)
            return 0;
        retVal = validateBirthString(input);
        if (retVal == 0)
            printf("#SYSTEM: Date isn't correct.\n");
        }
