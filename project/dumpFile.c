 ...
 * dumpFile.c
 *
 *        This module uses for reading all database files, validate, and keep
 *        in data structure. There're 3 files to read: 
 *        1) profile file 2) relation file 3) status file
 *      
...
/* LOCAL FUNCTION.
 * Open database file for read and return back.
 * If there isn't any file create new one.
 * Arguments:
 *      pOpen - file name that we want to open
 *      select - Select for text file[1] or binary file[2]
 * Return pointer of file after open.
 */
...
/* LOCAL FUNCTION.
 * Open database file for write data and return back.
 * Arguments:
 *      pOpen - file name that we want to open
 *      select - Select for text file[1] or binary file[2]
...
/* LOCAL FUNCTION.
 * Find data and then set program like there are user login in system.
 * Set for add another data into system.
 * Argument:
 *      mail - E-mail that wew ant to find
...
/* Subfunction of '_readStatusFile' read mail and date in file and validate.
 * Arguments:
 ...
 * validate and then keep in data structure.
 * Arguments:
...
void _readProfileFile(FILE* pFile)
    ...
    while(fread(pNew, sizeof(USER_T), 1, pFile) == 1)
        ...
        else if(validateUser(pNew) != 1)    /* Validate date*/
            {
            printf("#SYSTEM: Data has some information that invliad\n");
            printf("#SYSTEM: Cancel this data\n");
            if(pNew != NULL)
                free(pNew);
            }
        else
...
/* LOCAL FUNCTION.
 * Read all status and comment of user in status file.
 * Argument:
 *      pFile - File that want to read
 */
