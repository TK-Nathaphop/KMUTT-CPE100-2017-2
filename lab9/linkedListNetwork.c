...
typedef struct _vertex
    ...
    int dValue;                    /* sum of weights for shortest path so far
                                      to this vertex */
    struct _vertex * parent;       /* pointer to parent found in Dijkstra's
                                      algorithm */     
...
int printShortestPath(char* startKey, char* endKey)
    {
    VERTEX_T *pDummy = NULL;
    VERTEX_T * pFromVtx = findVertexByKey(startKey,&pDummy);
    VERTEX_T * pToVtx = findVertexByKey(endKey,&pDummy);
    VERTEX_T * pCurrentVtx = NULL;  /* Vertex that we consider */
    VERTEX_T * pVtxOfAdj = NULL;    /* Vertex in adjacent */
    ADJACENT_T * pAdj = NULL;       /* Adjacent that we consider */
    int weight = 0;
    ...
    /* Set number value greater than sum of all weight and put in queue */
    initAll();
    pFromVtx -> dValue = 0;
    while(queueMinSize() != 1)
        {
        pCurrentVtx = dequeueMin();
        /* Underflow, Exit from loop */
        if (pCurrentVtx == NULL)
            break;
        pCurrentVtx->color = BLACK;
        pAdj = pCurrentVtx->adjacentHead;
        /* Set all adjacent of white vertex to sum of weight and value of current
         * vertice (If it's less than value of white vertex) */
        while (pAdj != NULL)
            {
            pVtxOfAdj = pAdj->pVertex;
            if (pVtxOfAdj->color == WHITE)
                {
                if ((pCurrentVtx->dValue + pAdj->weight) < pVtxOfAdj->dValue)
                    {
                    pVtxOfAdj->dValue = pCurrentVtx->dValue + pAdj->weight;
                    pVtxOfAdj->parent = pCurrentVtx;
                    }
                }
            pAdj = pAdj->next;
            if(pAdj == NULL)
    ...
    /* If don't have parent, can't traversal  */
    if(pToVtx->parent == NULL)
        return -2;
    colorAll(WHITE);
    /* Traverse back from to vertex, and print path from start to the end */
    while ((pToVtx->color == WHITE) && (pToVtx->parent != NULL))
        {
        pCurrentVtx = pToVtx;
        pVtxOfAdj = pCurrentVtx->parent;
        while ((pVtxOfAdj != NULL) && (pVtxOfAdj->color != BLACK))
            {
            pCurrentVtx = pVtxOfAdj;
            pVtxOfAdj = pCurrentVtx->parent;
            }
        pCurrentVtx->color = BLACK;
        printf("%s", pCurrentVtx->key);
        if(pCurrentVtx != pToVtx)
            printf(" ==> ");
        }
    weight = pToVtx -> dValue;
    printf("\nWeight is %d\n",weight);
    return weight;
    }
